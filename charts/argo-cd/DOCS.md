...
| redis-ha | object | `{"enabled":true,"exporter":{"enabled":true},"haproxy":{"enabled":true,"metrics":{"enabled":true}},"image":{"tag":"6.2.4-alpine"},"persistentVolume":{"enabled":false},"redis":{"config":{"save":"\"\""},"masterGroupName":"argocd"}}` | This key configures Redis-HA subchart and when enabled (redis-ha.enabled=true) the custom redis deployment is omitted |
